      ...
      integer :: npolls               !           |ICRA pollutant counter
          ...
          if (nspu > 0) then
            ...
            do i = ob1, ob2
              ...
              if (cs_db%num_tot > 0) then
                ...
                if (npolls > 0) then
                  allocate (obcs(i)%hin%poll(npolls))
                  allocate (obcs(i)%hin_sur%poll(npolls))
                  allocate (obcs(i)%hin_lat%poll(npolls))
                  allocate (obcs(i)%hin_til%poll(npolls))
                end if
